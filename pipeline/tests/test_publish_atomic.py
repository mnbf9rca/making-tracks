...
def test_unsafe_region_or_version_is_refused_before_any_path_is_built():
    ...
    R.validate_path_components("united-kingdom", "20260715T120000Z")


def test_build_staging_writes_the_public_r2_shape(tmp_path):
    basemap = tmp_path / "united-kingdom.pmtiles"
    ...
    assert (root / "united-kingdom.pmtiles").read_bytes() == b"basemap"
...
def test_build_staging_replaces_stale_version_directory(tmp_path):
    basemap = tmp_path / "united-kingdom.pmtiles"
    basemap.write_bytes(b"basemap")
    stale = tmp_path / "stage/united-kingdom/20260715T120000Z/zone-catalog.json"
    ...
    root = S.build_staging(
        ...
        "20260715T120000Z",
        tile_arts=_arts(),
        manifest_obj={"schema_version": 1, "publish_version": "20260715T120000Z", "region": "united-kingdom"},
        basemap_path=basemap,
    )
...
def test_all_private_kind_ops_target_the_private_bucket_and_layout_is_distinct():
    ...
    with pytest.raises(R.LayoutInvalid):
        R.PublishPlan.for_version(bad, "united-kingdom", "20260715T120000Z", _arts(), basemap=True)
...
def test_for_version_uses_registry_jsonl_extension():
    ...
    assert [op.key for op in registry_ops] == ["registry/united-kingdom.jsonl"]
...
def test_existing_version_prefix_is_refused_even_with_a_ledger(tmp_path):
    ...
    class ExistingPrefixClient:
        ...
        def put_object(self, *, Bucket, Key, Body, IfNoneMatch=None):
            assert Key == "united-kingdom/publish.lock"
...
def test_prepared_multi_region_upload_flips_currents_then_merges_region_index(tmp_path):
    thumb_sha, thumb_body = _real_thumb_bytes()
    layout = _layout()
    uk_root = tmp_path / "stage" / "united-kingdom" / "20260715T120000Z"
    sub_root = tmp_path / "stage" / "united-kingdom_london" / "20260715T120000Z"
    for root, region in [(uk_root, "united-kingdom"), (sub_root, "united-kingdom_london")]:
...
def test_region_index_offline_dry_run_plan_serializes_the_planned_body(tmp_path):
    ...
    assert result.dry_run is True
    assert result.plan.ops[0].source_path is None
    assert json.loads(result.plan.ops[0].body)["regions"][0]["id"] == "united-kingdom"
...
def test_upload_recovers_a_stale_publish_lock(tmp_path):
    ...
    assert ("making-tracks-state", "united-kingdom/publish.lock", '"stale-etag"') in client.deleted
    assert ("making-tracks-state", "united-kingdom/publish.lock", '"fresh-etag"') in client.deleted
