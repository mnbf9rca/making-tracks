...
case "$destination_fields" in
  ...
  *)
    echo "regenerate-t2.10-about: destination must include id=<simulator-udid>" >&2
    exit 1
    ;;
esac
simulator_udid="${destination_after_id%%,*}"
[ "${MT_SIM_LOCK_UDID:-}" = "$simulator_udid" ] || {
  echo "regenerate-t2.10-about: simulator lock does not match destination" >&2
  exit 1
}

actual_xcode_version="$(xcodebuild -version)"
[ "$actual_xcode_version" = "$expected_xcode_version" ] || {
  echo "regenerate-t2.10-about: requires $expected_xcode_version; found $actual_xcode_version" >&2
  exit 1
}

xcrun simctl bootstatus "$simulator_udid" -b

only_testing_file="$(mktemp /private/tmp/t2.10-about-only-testing.XXXXXX)"
cleanup() {
...
printf '%s\n' \
  'MakingTracksUITests/MakingTracksCoreLoopUITests/testDoorInventoriesAboutCreditsAndMapAttributionIsInert' \
  'MakingTracksUITests/MakingTracksCoreLoopUITests/testCreditsStayGroupedAtAccessibilityTextSize' \
  > "$only_testing_file"
...
assets=(
  t2.10-about.png
  t2.10-software-licences.png
  t2.10-data-licences.png
  t2.10-about-ax.png
  t2.10-software-licences-ax.png
  t2.10-data-licences-ax.png
)
...
for asset in "${assets[@]}"; do
  ...
  [ -s "$source_path" ] || {
    echo "regenerate-t2.10-about: missing capture $source_path" >&2
    exit 1
  }
  ...
  [ "$pixel_width" = "1206" ] && [ "$pixel_height" = "2622" ] || {
    echo "regenerate-t2.10-about: $asset is ${pixel_width}x${pixel_height}, expected 1206x2622" >&2
    exit 1
  }
done

echo "$actual_xcode_version"
echo "XCTest XCUIScreen captures: 1206x2622 px (402x874 pt @3x), iOS 26.5 simulator"
