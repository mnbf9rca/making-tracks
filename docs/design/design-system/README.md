...
| Asset | What it shows |
|---|---|
| `t2.3-door-glyph-{before,after}-{default,ax}.png` + [`t2.3-door-glyph-evidence.md`](t2.3-door-glyph-evidence.md) | T2.3 implementation evidence at exact 390×844 default and AX5 dark canvases. Raised shared rows move from the 17pt literal to `rowRaised` 20pt/`.headline`; hairline rows move to `rowQuiet` 18pt/`.subheadline`; Explore quiet destinations keep 18pt but move from `.body` to `.subheadline` anchoring while deliberately retaining medium weight and no pulse. Their 24pt icon column now scales at AX5, and the scroll-advanced AX proof contains both quiet destinations without glyph/copy collisions. Scope/site 2 remains untouched for T2.8. |
