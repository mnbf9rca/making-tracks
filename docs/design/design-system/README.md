...
| Asset | What it shows |
|---|---|
| `t2.8-explore-door-{default,ax}.png` + `t2.8-explore-door-scope-{default,adjusted}.png` + `t2.8-explore-scope-{list-open,other-lists,other-lists-empty}.png` | T2.8 runtime UI-test evidence at exact implementation head `4c627c8d409a9259018e604449bd796709d9f3e9`: the contextual Explore scope picker, its populated and empty Other Lists child, and the stable Explore door whose icon alone changes when scope is adjusted. Every 1206×2622 PNG is paired with a same-basename `.txt` runtime measurement record; [`t2.8-implementation-captures.txt`](t2.8-implementation-captures.txt) records all SHA-256 hashes, exact simulator destination, and Xcode 26.6 (17F113). The 6/6 locked UI-test capture run completed in 140.640s. Measured contracts include 52pt default preference-row pitch, 184.67pt AX5 rows (above the 86pt minimum), 44pt Explore door/back/Loved targets, a 68.33pt selected-list row, and Clear scope fixed outside the list collection. Regenerate through the simulator lock with `MT_RELEASE_GATE_DESTINATION='platform=iOS Simulator,id=42D1482C-DE04-49AA-990D-1884ED9B855D' MT_RELEASE_GATE_DERIVED_DATA=/private/tmp/dd-codex4 ./docs/design/design-system/capture-t2.8-implementation.sh`. |
