...
| Asset | What it shows |
|---|---|
| `t2.9-settings-root{,-ax}.png` | T2.9 runtime UI-test evidence for the W-2 Settings root on the Snow surface. The default top-state capture shows all seven first-class groups and the selected Snow preset; the AX5 top-state capture shows the title, explanatory copy, group heading, and content-yielding first row without clipping. The same focused UI test then scrolls through every root row and checks each row's label, summary, minimum natural height, and containment. Each capture is 1206×2622 px (402×874 pt @3×). Captured from signed source head `36a7cb3f8154f4b9ff2fec3f96e796d5ed32a3e5` with XCTest `XCUIScreen.main.screenshot()` under Xcode 26.6 (17F113) on the locked codex2 iPhone 17, iOS 26.5 (23F77) simulator, with the status bar frozen at 09:41. Regenerate through the simulator lock with `MT_RELEASE_GATE_DESTINATION='platform=iOS Simulator,id=BACC2CF8-C1F8-4C92-B058-47B0AC0B128D' ./scripts/sim-lock.sh ./docs/design/design-system/regenerate-t2.9-settings.sh`. SHA-256: `5d17dffbaf50fc712575c58727ca93673a49b4aa7bbf5fee23ebbb1126802dd2` (default) and `855c60d94f879716d7b9c508330cd53e36d5adade2dcc51441706684e7d07c94` (AX5). See `t2.9-settings-evidence.md` for the capture ledger. |
