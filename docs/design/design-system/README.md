...
## Pre-phase wireframe proposals — ruled, awaiting delta validation

These issue-routed packets remain implementation-blocked until their required ruling and reviewer gates pass.

| Asset | What it shows |
|---|---|
| [`375-copy-id-menu.html`](375-copy-id-menu.html) + `375-copy-id-menu{,-ax,-copied,-copied-ax}.png` | **Evidence class: Deterministic fixture. Rob ruled the in-row flash; reviewer validation of the confirmation delta is pending.** #375's four exact 390×844 frames show the default and AX native-menu states before and after activation: `Copy ID` follows `Add to list`, copies only the raw `mt1_…` value, changes in place to disabled `Copied` plus a checkmark for one second while VoiceOver announces success, then dismisses. No toast or inline ID appears. The menu geometry is illustrative and system-owned; row order, payload, feedback, and bounded timing are the decision. [`375-copy-id-menu-ruling.md`](375-copy-id-menu-ruling.md) records feasibility, alternatives, constraints, accessibility, reproduction, and all four SHA-256 values; SHA-256 is the byte-reproduction oracle. Regenerate with `./scripts/render-375-copy-id.sh`. |
