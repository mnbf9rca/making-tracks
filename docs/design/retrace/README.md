...
- `retrace-filter-selection-affordance.svg` / `.png` - expanded map-surface
  picker for choosing among many list and place-type filters.
- `retrace-filter-selection-affordance-axxxl.svg` / `.png` - the same picker at
  accessibility size, with wrapped chips and fewer visible rows rather than
  compressed text.
...
- The filter-selection render shows list and type filters as one map-surface
  scope picker. Active choices remain visible as chips while the expanded picker
  is open. List rows use check controls; type choices use chips. The primary
  action reports the scoped visit count, not a hidden or omitted count.
...
'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome' --headless --disable-gpu --hide-scrollbars --force-device-scale-factor=1 --disable-background-networking --disable-component-update --disable-sync --no-first-run --no-default-browser-check --user-data-dir=/private/tmp/chrome-retrace-filter --screenshot=docs/design/retrace/retrace-filter-selection-affordance.png --window-size=390,844 file://$PWD/docs/design/retrace/retrace-filter-selection-affordance.svg
'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome' --headless --disable-gpu --hide-scrollbars --force-device-scale-factor=1 --disable-background-networking --disable-component-update --disable-sync --no-first-run --no-default-browser-check --user-data-dir=/private/tmp/chrome-retrace-filter-axxxl --screenshot=docs/design/retrace/retrace-filter-selection-affordance-axxxl.png --window-size=390,844 file://$PWD/docs/design/retrace/retrace-filter-selection-affordance-axxxl.svg
```
