...
### The Mac (since 2026-07-29): M5 Pro, per-seat simulators, two concurrent gates

Rob's Mac was replaced 2026-07-29 (M5 Pro, 18 cores, 48 GB; repo moved to `~/git/making-tracks`).
iOS-line consequences, canonical on `ios` (`docs/ios-gate-ledger.md` "Host Gate Seats" and
`scripts/sim-lock.sh`), summarised here so develop-side readers stop assuming the old topology:
...
- Locking is per-simulator plus a global cap of **two concurrent full gates**
  (`MT_GATE_MAX_CONCURRENT`, default 2). The old single-shared-simulator serialization is retired.
...
- Result bundles: failed-run `.xcresult`s are never auto-deleted (evidence law); success cleanup is
  manual pending #546. Any weekly-cleanup runbook claim on this branch is superseded by that law.
