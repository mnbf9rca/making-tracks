...
| **Description sidecars** | **Bundled (mandatory)** | `descriptions/10/{x}/{y}.json` (codex4 #173). |
| **Image-index sidecars** | **Bundled (OPTIONAL — part of "include images")** | `images/10/{x}/{y}.json` (#158). Rides *with* thumbs (an image-index entry is useless offline without its thumb) — a without-images pack carries neither. |
| **Image thumbs** | **Bundled (OPTIONAL)** | `thumbs/{sha[:2]}/{sha}.webp`, content-addressed; the "include images" toggle (WP-IMG-B2). |
| **Search index** | **Bundled + standalone** | Per-region names + zone names; local search (B9). |
...
- **Every bundled content type is a `{sha, bytes}`-listed object.** The tile `manifest.json` is **frozen**
  ...
  PACK-DESCRIPTOR file** (sidecars cannot ride the manifest — region-model §6). Refresh runs `updatePlan`
  **sha-skip** over both → an unchanged object (same sha) is not re-downloaded (**free delta**).
- **Per-content-type delta:**
  ...
  - **Basemap** — the monolithic `pmtiles` whole-file re-download is the "GB every time" gap; the design
    answer (WP-RM §6, pending #177) is **per-cell content-addressed basemap objects** (delta free, same
    grid as tiles), over range-based pmtiles sync. **Confirm on #177 merge.**
...
- **Incremental-persist is MANDATORY** — stream each **verified** object to the content-addressed store
  **as it arrives** → **constant memory bound** regardless of pack size. (RAM-buffering the whole pack is
  unacceptable — the device was jetsam-killed at far smaller working sets.)
- **Resume is free:** an interrupted download resumes **object-granular** via `updatePlan` sha-skip.
...
|---|---|
| Completeness invariant; packs/zones; delta strategy | WP-RM (`2026-07-18-wp-rm-region-manager.md`, PR #177, OPEN) |
| **Offline download engine + content-addressed pack store** | **WP-B7 (`2026-07-17`, PR #149, MERGED)** + region-model §3/§5 (#131) |
...
| Search index (local, names + zones) | B9 (`2026-07-18-wp-b9-search.md`) |
