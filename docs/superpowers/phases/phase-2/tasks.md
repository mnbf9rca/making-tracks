...
| **AC2.4** | `MapDoorRowIconGlyph`'s 17pt literal resolves through `rowRaised`/`rowQuiet`; the hand-rolled quiet Explore glyph (`MapDoorShell.swift:689`) adopts `rowQuiet` and loses the duplicate `quietDestination` member; and the Scope glyph (`:630`) preserves its distinct P2R-4 `scopeControl` metric while re-anchoring it to `.button`. **#520 closes and Phase 1 acceptance completes at 35/35** on site 1 alone; sites 2–3 are anti-drift, not AC15 debt (see T2.3). Point-of-use coverage fails if any of the three bypasses its assigned vocabulary again — a definition-only test is insufficient. | P2R-2(b), P2R-4, #520 Done-when | ruled sizes; three-site scope **reviewer-derived**; exact Scope anchor **T2.1 contract choice** |
...
`ExploreSurfaceIconGeometry.scopeControl` as the P2R-4 Scope-control metric and change its live
`@ScaledMetric` anchor from `.body` to `.button`, matching the 15pt/600 control label. Do not
substitute `rowRaised`: its equal `20` is paired to the 17pt list-row title and therefore scales
under a different semantic contract. Update the enum's comment to name the surviving Scope metric.
T2.2 is forbidden from doing any of this because its host-only gate cannot compile the app-target
consumers.
