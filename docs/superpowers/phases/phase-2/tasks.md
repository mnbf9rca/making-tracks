...
- **#579 — atomic-publish test/API skew on `ios`**, PR open — draft #597: #463 copied three multi-region tests without the `develop`-only production APIs or callers they exercise; removing that exact test-only hunk restores 43/43 atomic-publish and 695 passed / 1 skipped across the full pipeline suite. Recurrence prevention is filed as #596.
...
| **#579** — atomic-publish test/API skew | codex3 | draft PR #597 open; recurrence guard #596 filed |
