...
| **AC2.4** | Every icon on the door surfaces resolves through `IconRole`, and `ExploreSurfaceIconGeometry` no longer exists — so no second home for 20/18 survives. Point-of-use coverage fails if any of the three sites bypasses the vocabulary again; a definition-only test is insufficient. **#520 closes and Phase 1 acceptance completes at 35/35.** *(Which sites, why each is in scope, and why the 35/35 claim rests on the `MapDoorRowIconGlyph` site alone: T2.3's brief.)* | P2R-2(b), #520 Done-when | ruled; the three-site scope is **reviewer-derived** |
...
**Then delete `ExploreSurfaceIconGeometry`** (`Iconography.swift:48`–`:52`). After your migration it has no consumers. Its doc comment claims the figures "are not new icon roles"; P2R-2(a) made that false, so the enum goes rather than being re-commented. **The deletion is yours and not T2.2's — the reason lives once, in T2.2's row; do not restate it, read it.**
