...
Drafted by reviewer; landed by planner after independent verification. Every load-bearing tree claim was re-checked against `ios` at `9fa91d4` before landing: the `trail` rename (no `trackLine` symbol survives), `disabledAlpha`/`pressScale` live in sheet and spec, `hiddenPinColor`'s raw literal at `PinLayers.swift:6`, the shipped Explore surface and vestigial LayersSheet plumbing, the five `isDefault` pins, the coverage storage key, the spec's 19×19 door-pill row (OF3 is real), and the 17-beside-15 and `accentDeepContainer` table rows. The identifier blast radius re-grounds at **37** occurrences (reviewer's prefix grep; planner's 36 excluded the bare `map.layers` at `UITests:492`) — re-count at claim; it is a blast-radius indicator, not a contract. Two sites no per-identifier grep sees are named in T2.8's brief: the bare-identifier assertion (`UITests:492`) and the `hasPrefix("map.layers.")` helper (`UITests:588`), which after a rename does not fail — it silently stops selecting.
...
- **OF1 is with Rob** (raised in planner's chat, per the gate law); OF2's stated default stands; OF3 carries to the next session (and is later resolved by the designer ruling recorded in #594); OF4/OF5 are W-1's to draw and Rob's to rule.
...
| **AC2.4** | Every door-surface icon resolves through its assigned vocabulary: the #520 glyph and the quiet Explore glyph through `IconRole`, the Scope glyph through P2R-4's `scopeControl` metric. `ExploreSurfaceIconGeometry.quietDestination` is gone; `scopeControl` survives as a distinct figure — **numeric equality does not collapse two semantic figures**. Point-of-use coverage fails if any of the three sites bypasses its assigned vocabulary again; a definition-only test is insufficient. **#520 closes and Phase 1 acceptance completes at 35/35.** *(Which sites, why each is in scope, why 35/35 rests on the `MapDoorRowIconGlyph` site alone, and which task owns the Scope anchor: T2.8's brief.)* | P2R-2(b), P2R-4, #520 Done-when | ruled sizes; three-site scope **reviewer-derived**; the Scope anchor is a **T2.1 contract choice** |
...
| The identifier rename is a wireframe annex detail. | It is a 37-occurrence blast radius (prefix grep, both suites) across `ios/App/UITests` and `ios/App/Tests`: `show-hidden` ×9, `show-all-categories` ×8, `coverage-shading` ×7, `show-saved` ×4, `category.*` ×4, plus loose matches. One commit, nothing in flight — exactly as P2R-6 says, and the reason it says so. |
...
| The closed set's pressure is 20 and 18. | **Resolved by the designer ruling recorded in #594.** The table now snaps `ia-doors`' drawn 19×19 door-pill icon to the shipped `IconRole.inline` = **15×15**, whose visual tolerance was proven by review acceptance; no third figure remains. See **OF3**. |
...
- **OF1 — RESOLVED (designer ruling, relayed by Rob, 2026-07-31).** Ruled verbatim: *"The coverage-shading switch lives only in the map's filter picker behind the Explore door; Settings' Coverage group is data only — published regions, sources, extents — with no shading switch in it."* The designer's own grounds, kept because they decide future cases of this shape: *"That's not a new call, it's three existing rulings agreeing: the shading toggle is one of the ruled scope rows (it's in the pinned defaults — hidden OFF, saved ON, coverage ON — and persists under P2R-5), the doors law says frequent map-facing choices live behind the door while set-and-forget lives in Settings, and duplicating one control across two surfaces is precisely the two-doors-to-one-room drift #266 was killed for."* W-2 draws the Coverage group as data-only and cites this entry. The flag's original text follows as history. **(was: owner Rob, via planner; blocked T2.8/T2.9 acceptance readings.)** P2R-5 ratifies coverage shading as one of four persisted **scope** choices, default ON, in the Explore picker. #472 and W-2 give Settings a **Coverage** group. If both ship the same toggle, one control has two homes and they will drift; if the Settings group means coverage **data** (sources, downloaded coverage, the #360 territory) it is not a conflict at all. **Reviewer's reading, offered for correction:** the *shading* toggle is scope and stays in the picker; Settings' Coverage group covers coverage **data and downloads** and carries no shading toggle. Confirm at the W-2 gate, where it becomes visible.
...
- **OF3 — RESOLVED (designer ruling relayed by Rob 2026-08-01; #594).** The shipped `IconRole.inline` value wins: the spec's door-pill row is amended from the drawn 19×19 to 15×15, and its provenance cell records the R11 snap and review-proven tolerance. The frozen render stays untouched. The carried obligation is discharged without minting a sixth role or retaining a one-site literal.
...
- **A second slot is planner's call, not mine.** T2.8 re-points **37 `map.layers` occurrences** across the UITest and unit suites while dismantling a chip surface. It is mechanical and fully verifiable by grep, so I have not spent a slot on it — flagging it rather than quietly consuming budget.
...
**(a) The two row roles.** `IconRole` (`Iconography.swift:26`–`:43`) is three cases — `hero` 22 / `inline` 15 / `accessory` 11 — each with a paired `TypographyRole` and `UIFontMetrics` scaling. Add `rowRaised` = 20 anchored to `TypographyRole.listRowTitle` and `rowQuiet` = 18 anchored to `TypographyRole.button`, exactly as T2.1's table fixes them. State those consumed anchors in the PR body; they are no longer choices for this builder to reopen.
...
**`ExploreSurfaceIconGeometry` holds one duplicate and one distinct figure.** `Iconography.swift:48`–`:52` declares `public enum ExploreSurfaceIconGeometry { scopeControl = 20; quietDestination = 18 }` under the doc comment *"Ratified pairing constants for the Explore surface; these are not new icon roles."* A8's builder parked both figures there **correctly** — R11 had closed the set, so constants were the only legal home. P2R-2(a) makes only `quietDestination` a second home for `rowQuiet`. The numerically equal `scopeControl` 20 belongs to P2R-4's Scope-control row; T2.1 anchors it to `.button` as a contract choice matching that row's 15pt/600 label, not to P2R-2's `rowRaised`/17pt list-row contract.

**Do not edit the enum in this task, and this is a build trap rather than a preference.** Its consumers are in the **app** target (`MapDoorShell.swift:581`, `:680`), which `cd ios && swift test` never compiles — so removing a member here passes your host gate and breaks the app build for whoever runs next. **T2.3 migrates the quiet consumer and removes only `quietDestination`; `scopeControl` and its live consumer pass untouched to T2.8.** Mark `quietDestination` deprecated with a pointer to `rowQuiet` if the language makes that cheap, and name both hand-offs in your PR body.
...
Treat them differently because their contracts are different. Migrate `ExploreQuietDestinationRow`
to `rowQuiet` and remove only `ExploreSurfaceIconGeometry.quietDestination`. Preserve
`ExploreSurfaceIconGeometry.scopeControl` through this task and leave its live `.body`-anchored
consumer untouched for T2.8. Do not substitute `rowRaised`: its equal `20` is paired to the 17pt
list-row title and therefore scales under a different semantic contract.

**The quiet migration is NOT mechanical, and here is the part to decide deliberately.** `IconRoleModifier` resolves its weight from `@Environment(\.materialControlSymbolWeight)` (`Iconography.swift:62`, `:76`), which is what drives A5's **symbol-weight press pulse**. The hand-rolled quiet site pins `.medium` and therefore sits outside that environment today. Adopting `.iconRole(.rowQuiet)` inside a `Button` newly enrolls it. That is a **behaviour change on a live surface**, not a rename: state whether you kept or accepted it, with a render either way, and if you suppress it say how. R9's family is why the pulse exists; a row glyph is not obviously one of its subjects. The Scope-control glyph stays outside `IconRole` and keeps its present weight behaviour unless a later ruling says otherwise.

**This is a visible change** — 17 → 20/18 on live door rows. Ship before/after renders at 390×844 plus an AX variant with measurements beside the images, per the evidence law. `AppShellTests.swift` asserts the #520 glyph in five places (`:1119`, `:1202`, `:1275`, `:1293`, and the point-of-use coverage #520 requires) — re-point them, and keep the coverage assertion **point-of-use**: #520's Done-when states in terms that a definition-only `IconRole` test is insufficient. Extend that coverage to sites 2 and 3, or the same drift returns on the surface DS-3 is about to rebuild.
...
- **Follow-up #583 status:** draft PR #587 — `wp-583-followup` (codex4); baseline host suite 516/516 and full host gate 341/341 green
...
1. **The identifier rename happens once, in one commit, with nothing in flight against it** (P2R-6). Take the names from W-1's annex — do not invent them. The blast radius is **37 occurrences** of `map.layers` across `ios/App/UITests` and `ios/App/Tests` (re-count when you claim): `show-hidden` ×9, `show-all-categories` ×8, `coverage-shading` ×7, `show-saved` ×4, `category.*` ×4, **plus two sites a per-identifier grep cannot see**: the bare-identifier assertion `XCTAssertFalse(app.buttons["map.layers"].exists)` (`MakingTracksCoreLoopUITests.swift:492`) and — the one that bites — the **prefix-matching branch** `if identifier.hasPrefix("map.layers.")` (`:588`). That branch is **the only test in either suite pinning P2R-4's ratified 86pt AX Scope-row minimum** (`:591`, the sole `86` assertion), inside a loop whose other two identifiers (`explore.row.settings`, `explore.row.about`) are already A1-convention and are NOT renamed. After the rename the prefix matches nothing, the 86pt branch never executes, and the loop **stays green** — the un-renamed rows keep its 44pt assertion honest — so the suite silently degrades from "AX Scope rows ≥ 86pt" to "rows ≥ 44pt", deleting the enforcement of a figure ratified in T2.1 **in the same phase, without a single red test**. **Acceptance for the rename is therefore twofold: (1) a grep for the OLD prefix returns zero occurrences in app code and helpers outside assertions whose purpose is to prove absence; (2) the 86pt assertion provably still executes — neuter it once (P2R-4's figure minus one) and show it red, then restore.** The new identifiers passing their tests is not sufficient evidence, because this failure mode is green. **Re-point tests; do not delete them to get green** — each one is coverage of a control that still exists. The vestigial plumbing names (`layersSheetVisibility`, `updateLayersSheetVisibility`, `MapScreen.swift:3882`–`:3890`) name a sheet that was already dismantled; rename them in the same commit or say why not.
...
**Also in this row:** the scope-active indicator on the Explore door, consuming T2.7's predicate (AC2.8); the quiet Clear-scope row (AC2.9); and the P2R-4 geometry as **ratified defaults** — a deviation is its own gate with the delta named. Re-home the surviving `ExploreSurfaceIconGeometry.scopeControl` 20 as a separately named Scope-control metric, anchor its live `ExploreScopeControlGlyph` consumer to `.button`, and retire the old enum member only after that app-target migration. The figure remains distinct from `rowRaised`; prove the 20-default/30-AX pair against W-1's geometry.
...
- **OF3's former 19×19 door pill icon — RESOLVED by #594's designer ruling.** The spec now carries 15×15 (`IconRole.inline`) with the R11-snap provenance; the frozen render remains the evidence it was, and this item is retained only as closeout history, not as an active carry.
...
- **#579 — atomic-publish test/API skew on `ios`**, complete — merged as `da5ce2b` (#597) and closed: #463 copied three multi-region tests without the `develop`-only production APIs or callers they exercise; removing that exact test-only hunk restored 43/43 atomic-publish and 695 passed / 1 skipped across the full pipeline suite. Recurrence prevention is filed as #596.
...
| **OF3** — former drawn 19×19 door pill vs shipped `IconRole.inline` = 15 | discharged | #594 designer ruling; spec amended to 15×15 with honest provenance |
| **#584** — evidence-pipeline findings | planner | boot-on-demand script edits; the class narrowing has already landed as law |
| **#579** — atomic-publish test/API skew | discharged | merged as `da5ce2b` (#597); issue closed; recurrence guard #596 filed |
