...
- **#579 — atomic-publish test/API skew on `ios`**, tests green on `fix-579-atomic-publish`: #463 copied three multi-region tests without the `develop`-only production APIs or callers they exercise; removing that exact test-only hunk restores 43/43 atomic-publish and 695 passed / 1 skipped across the full pipeline suite.
...
| **#579** — atomic-publish test/API skew | codex3 | tests green on `fix-579-atomic-publish`; PR next |
