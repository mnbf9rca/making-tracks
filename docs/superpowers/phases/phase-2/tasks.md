...
| **AC2.4** | `MapDoorRowIconGlyph`'s 17pt literal resolves through `rowRaised`/`rowQuiet`, **and** the two hand-rolled Explore glyphs (`MapDoorShell.swift:630`, `:689`) adopt the roles so `ExploreSurfaceIconGeometry` can be deleted — after T2.3, every door-surface icon resolves through `IconRole` and no second home for 20/18 survives. **#520 closes and Phase 1 acceptance completes at 35/35** on site 1 alone; sites 2–3 are anti-drift, not AC15 debt (see T2.3). Point-of-use coverage fails if any of the three bypasses the vocabulary again — a definition-only test is insufficient. | P2R-2(b), #520 Done-when | ruled; the three-site scope is **reviewer-derived** |
...
**`ExploreSurfaceIconGeometry` already holds both figures, and minting the roles makes it a second home.** `Iconography.swift:48`–`:52` declares `public enum ExploreSurfaceIconGeometry { scopeControl = 20; quietDestination = 18 }` under the doc comment *"Ratified pairing constants for the Explore surface; these are not new icon roles."* A8's builder parked the figures there **correctly** — R11 had closed the set, so a constant was the only legal home. P2R-2(a) moves that law, and the moment your two roles exist that comment is false and `DesignSystem` holds two answers to what a row glyph measures, with nothing failing when they drift. That is T1.11's defect verbatim.

**Do not delete the enum in this task, and this is a build trap rather than a preference.** Its only consumers are in the **app** target (`MapDoorShell.swift:581`, `:680`), which `cd ios && swift test` never compiles — so deleting it here passes your host gate and breaks the app build for whoever runs next. **T2.3 migrates the consumers and deletes the enum as its final step.** Mark it deprecated with a pointer to the roles if the language makes that cheap, and name the hand-off in your PR body.
...
**Sites 2 and 3 — the hand-rolled Explore glyphs.** `ExploreScopeControlGlyph` (`:621`–`:646`, via `:581`) and `ExploreQuietDestinationRow` (`:674`–`:700`, via `:680`) each render `.font(.system(size: iconSize, weight: .medium))` with `iconSize` an `@ScaledMetric(relativeTo: .body)` over `ExploreSurfaceIconGeometry.scopeControl` = 20 and `.quietDestination` = 18. **These are not AC15 violations** — the *figures* come from `DesignSystem`, and AC15 binds a view that **defines** a literal — which is why the Phase 1 acceptance record is unaffected and **35/35 still follows from site 1 alone**. They are here because `rowRaised`/`rowQuiet` express exactly these two figures anchored to a **paired typography role**, while these sites anchor to `.body` and hand-apply the weight. Leaving them is how the drift starts.

**Then delete `ExploreSurfaceIconGeometry`** (`Iconography.swift:48`–`:52`) — T2.2 is forbidden from doing it because its host-only gate cannot compile these app-target consumers. After your migration the enum has none. Its doc comment claims the figures "are not new icon roles"; P2R-2(a) made that false, so the enum goes rather than being re-commented.

**The migration is NOT mechanical, and here is the part to decide deliberately.** `IconRoleModifier` resolves its weight from `@Environment(\.materialControlSymbolWeight)` (`Iconography.swift:62`, `:76`), which is what drives A5's **symbol-weight press pulse**. Both hand-rolled sites pin `.medium` and therefore sit outside that environment today. Adopting `.iconRole(…)` inside a `Button` newly enrolls them — a scope row's glyph would pulse on press where it does not now. That is a **behaviour change on a live surface**, not a rename: state whether you kept or accepted it, with a render either way, and if you suppress it say how. R9's family is why the pulse exists; a row glyph is not obviously one of its subjects.
