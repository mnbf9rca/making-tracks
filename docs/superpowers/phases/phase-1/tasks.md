...
**If a required automated-review check reports SKIPPED or ERRORED, the PR body carries its run id, conclusion, timestamp and annotation count — before the review-clean transition, not after.** A grey check with no accounting is **unreadable**: cap exhaustion, a service outage and a suppressed review all look identical from the outside, and only the first is a thing a merger may wave through. The reason this is a rule rather than a reminder is where the failure happens — the clause applies at the *end* of a long build, when the check goes grey and the body is already written, so it is never front-of-mind at the moment it binds. **Three of the first four bodies in one wave missed it**; a rule that fires before the transition costs less than a fourth reviewer reminder.
...
- **Status:** merged as `9df09a58` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **Owner:** codex1
- **Review tier:** `sourcery` + `opus`
- **Status:** merged as `062d89a7` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **Owner:** codex3
- **Review tier:** `sourcery` + `opus`
- **Status:** merged as `7ff6de31` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **Status:** merged as `856c0377` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **Owner:** codex2
- **Review tier:** `sourcery` + `opus`
- **Status:** merged as `fed0aff5` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter); overlap contract restated in R10 form and R9 press feedback enforced by follow-up **#505** (merged `8259ea0a`).
...
- **Owner:** codex4
- **Review tier:** `sourcery` + `opus`
- **Status:** merged as `04e01a94` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **Owner:** codex4
- **Review tier:** `sourcery` + `opus`
- **Status:** merged as `580cf4c1` — final state recorded by the merger, on fable's authority (the `Status` field is the builder's until handoff and the merger's record thereafter).
...
- **R9 — Enabled interaction feedback must not alpha-composite a semantic token pair below AA — geometry, never whole-control opacity.** Ratified verbatim. **Scoping, in the designer's own terms:** *enabled* leaves disabled-state opacity legitimate — a disabled control is supposed to read as unavailable and AA does not govern it; *semantic token pair* leaves imagery alone — a photograph dimming under a press is not a token pair and this does not reach it. What it forbids is the whole-control fade as a press treatment, because compositing foreground and background uniformly toward the backdrop moves **both** and compresses the ratio between them. Press feedback is geometry: scale, inset, shape. → T1.3, T1.12, AC2.
  - **Historical consequence at the merged T1.12 head `3586e15d7497ee97bf5e196511f9ba185ad8b470` — verified, not assumed.** `MaterialChipStyleBody.controlOpacity` and `MaterialButtonStyleBody.controlOpacity` (`3586e15d7497ee97bf5e196511f9ba185ad8b470:ios/Sources/DesignSystem/ControlStyles.swift:282` and `:324`) both returned `configuration.isPressed ? 0.78 : 1` for the **enabled** state, applied to the composed control at `:270` and `:307`. *(Re-grounded to the head where it was true — codex2's formulation, verified. The original text said "in the merged tree" with line numbers their own fix invalidated; a consequence recorded against a moving tree stops being checkable the moment the tree moves.)* On Snow's filled chip that takes the `accentContrast`-on-`accent` pair from **6.13:1 at rest to 3.88:1 while pressed** over `surface`, 3.92:1 over `ground` — below AA for the 12pt/600 label. The filled button carries the identical treatment. *(Arithmetic in sRGB gamma space, which is how the layer composites; it is a calculation, not a gate run.)*
...
- **R10 — Derived hit targets tile; they do not overlap.** Ratified verbatim: *in multi-row flows of distinct-action controls, derived hit targets **tile** (abut, never overlap); each outset is `min(11pt, gap/2)` per side — vertical target = row pitch, horizontal to half the inter-chip gap between distinct-action neighbours, full 11pt only in free space (single rows, run edges, margins). Every screen point maps to exactly one control: **ambiguity is forbidden, not proximity**.* Cited to iOS precedent — keyboard tiling, dense-row pitch-as-target. → T1.12 (supersedes its overlap contract), **T1.13** (the API that makes it possible), DS-3 #469.
  ...
  - **The API this ruling assumes is supplied by T1.13 / #509.** `MaterialChip` accepts optional per-edge neighbour gaps, performs the `min` arithmetic internally, and exposes `MaterialChipGeometry` so a consuming layout can reason about pitch without hardcoding. Its unconstrained expansion is **up to 11pt per side on each axis to reach 44pt** — dimension-aware, so an axis already at or above 44pt expands by nothing — and reversible distinct-action neighbours tile to their shared gap. *(codex2's formulation, verified. The bullet previously said the API did not exist; T1.13 merged mid-phase and a record that describes an absent API outlives its own truth.)*
...
- **R11 — `IconRole` ships this phase; the icon scale becomes vocabulary rather than scattered literals.** Ratified. **The rationale is the evidence, and it is worth carrying because it decides future cases:** a builder invented a parallel icon scale the moment the vocabulary was missing, which is what every future surface will do — DS-3, DS-6 and DS-9 are all icon-dense — and scattered cited literals are precisely the debt the design system exists to kill. **AC35's carve-out is an escape hatch, not a home.** → T1.14, AC14, AC15, AC35.
  ...
  - **RESOLVED — the conditional fired.** `.action` was ratified as **R12** before T1.14 was claimed, so T1.14 migrated the 13pt/600 site in the same pass. **T1.8's AC35 exception list is empty**: the 22pt hero glyph is owned by `IconRole.hero`, the 13pt/600 Retrace type by `TypographyRole.action`. Point (4)'s original *"shrinks by one"* is now literally true. **The correction above is kept rather than deleted** — a conditional that fired is history, and history stacks rather than overwrites; a reader who meets only the resolved sentence cannot tell it was ever in doubt, and the doubt is what produced the escape-hatch clause that made this outcome possible.
...
- **R12 — `.action` is a typography role at 13pt/600.** Ratified: **`.action` = 13pt/600, Dynamic-Type-scaled**, ratified by `ia-doors.html`'s `.mt-ia .action` rule (`:428`–`:431`) together with its 3pt gap, as drawn on the Tracks door's Retrace cue (`:753`). **A render-ratified figure, not new taste.** R11's reasoning is accepted as stated: the role API stays closed, and a further role still requires a session. → T1.14, AC35.
  - **Provenance correction, recorded because the citation is the part nobody re-derives.** The ratification message named `coherence.html`. **The figure is `ia-doors`'.** `coherence.html`'s own `.coh .act` is **15px/600** — the place-card action bar, a different element, and the one that pairs with the ratified 17px icon. Citing coherence here would have created a live contradiction at the checkable citation: a reader following it finds 15, and the obvious repair is the destructive one — moving the Retrace cue to 15. **Corrected against the tree, not second-guessed on taste.**
  - **Consequence for AC35.** R12 empties T1.8's exception list in combination with R11 — see the resolution note under R11's accounting correction.

### Rulings from the 2026-07-26 design session

Source for R13–R16: the **2026-07-26 design session** (designer + Rob), the immutability law's
sanctioned channel for amending a ratified spec. Landed in the spec by #524 (`13244148`) and its
follow-up; implemented by the **amendment wave** ([`amendment-wave.md`](amendment-wave.md), #526).

- **R13 — Door rename: *Tracks* → *Journal*.** *Making Tracks* stays the brand; **"tracks" leaves the chrome as a UI noun**; the footprints icon survives; the subtitle is unchanged. → A1, spec §2.
  - **Independent corroboration, recorded beside the ruling because a future reader will ask whether the rename was necessary or merely taste.** A fresh-eyes cold read of the built surfaces — clean context, no spec, no rulings, no knowledge of this session — returned as its finding 6: *"'Tracks' sounds like GPS recording, but it actually means your personal visit history and saved lists — a look-back feature, not a record-now feature."* It reproduced the misreading `PRINCIPLES.md` → Product 6 predicts (*"'Tracks' invites that misreading"*) **with the corrective subtitle visible on screen**. The two processes ran in the same hour, in opposite directions — one from design intent, one from a reader who had never seen the intent — and **neither influenced the other**. A cold reader confusing themselves in the predicted direction despite the mitigation is stronger evidence than any argument for the rename could be.
- **R14 — Door rename and collapse: *World* → *Explore*, opening Scope directly.** No intermediate row list; **Settings and About become quiet bottom rows** on the Scope surface, the gear the more prominent of the two; the **Search slot sits at the top** when DS-11 lands, rendering nothing until then per R2. → A1, spec §2.
- **R15 — State morphology is component law.** **ON = filled pill + filled glyph; OFF = tonal + outline glyph; momentary verbs = quiet text.** **Never-colour-alone applies to state**, not only to seen-state and progress. The place card is re-rendered under this rule. → A2, A8, spec §5.
- **R16 — States are not buttons: §5 gains a fourth component category.** Ratified reading **D** of the R15-versus-§5 conflict. **`state toggle` — a control reporting a persistent fact.** Its form law is R15; it sits **outside the filled-action budget**, which continues to govern **actions only**. The reasoning is ratified verbatim and lands in the spec in these words: ***"states are not buttons — the budget exists to stop screens shouting competing imperatives; an ON state is not an imperative, it is a fact the control is reporting; three true facts are not three CTAs."*** → A2, A8, spec §5.
  - **The conflict R16 dissolves.** R15 makes ON a filled pill; §5 caps filled at one per screen. A place that is saved **and** seen **and** loved has three ON states at once, so both ratified statements could not hold on that card. R16 fixes the **taxonomy** rather than carving an exception into the budget — every future control cluster inherits the answer instead of asking for its own carve-out.
  - **Rider — an ON pill fills with the state's own semantic token.** Within a state cluster: **seen** fills `accent`, **loved** fills the ratified `love` row, **saved** fills `accent`'s tonal-strength companion. **Exact values are the re-render's job to prove against the AA gate.** Purpose: three ON pills read as three differently-toned **facts**, not three copies of the CTA colour, so the screen's single filled **action** stays unmistakable beside a fully-lit cluster. Morphology still carries the state signal — fill plus glyph, never colour alone — and the tones prevent CTA impersonation.
  - **Reading B was rejected on evidence, not on taste.** B would have kept containers tonal and let the **glyph alone** carry ON. The cold read's **number-one ranked confusion**, unprompted, was this exact control bar: *"the Save/Seen/Hide trio's inconsistent styling — I can't tell if all three are stateful toggles or if only Seen is."* **State legibility riding on the smallest mark on the control is precisely the defect R15 exists to fix**, so B solved the budget by reintroducing the problem.
...
- **The same amendment lifts `ia-doors.html`'s load-bearing numbers into a component-metrics table in the spec** — chip 12pt/600 with its 11px icon, the 22/9/4 geometry, the door order, the Tracks hierarchy. The designer's reason generalises past this one table and is worth carrying verbatim: **numbers belong in tables; renders certify feel; information that only exists as pixels gets under-read.** That is the same finding the render-set rule in *Operating rules* addresses from the reader's side — this addresses it from the source's side, and only the second one actually removes the trap.
  - The fourth instance is the argument's strongest and it came from the design authority itself — the ratification message for R12 cited `coherence.html` for a figure `ia-doors.html` ratifies, and the wrong file was plausible precisely because coherence carries its own action element at 600 weight, at 15px rather than 13px. **A wrong citation from the right family survives every check that is not a tree read.**
...
- **1 — A9's press inset becomes a scaled metric.** Ratified by Rob, verbatim: ***"The inset becomes a scaled metric anchored to the control's paired typography role — same law as icons, which already scale with their labels. Perceptibility of press feedback must be constant across Dynamic Type, not constant in points."*** The shipped `1pt` literal is a **legitimate way-station**, and the migration is one line because `IconRoleModifier` already anchors `@ScaledMetric` to a paired role. *The evidence that produced the ruling: the inset measured **+3 device pixels at both default and AX**, so its perceptibility fell as type grew — backwards for the users most likely to need visible feedback.*

- **2 and 3 together — the agenda question. Two ratified figures press `IconRole`'s closed set from opposite directions, and the session should meet them as one decision rather than two requests.**
  - **From below:** `ia-doors` ratifies **20px raised** and **18px quiet** row glyphs. The closed set (22/15/11) expresses neither, and R11 point (2) forbids minting a fourth role without session ratification — so the door rows ship a `17pt` literal **declared non-compliant** (`MapDoorRowIconGlyph`, #529) pending this.
  - **From above:** `coherence` ratifies a **17px icon beside a 15px label**, with a live consumer in `MaterialControlLabelStyle`. Recommended disposition already recorded under R11: it stays **outside** the scale as a ratified **pairing constant**, on the generalisation that *"the table ratifies, the API carries"* covers **constants as well as scales**.
  - **The question is therefore not "add a role" but whether three was the right closure at all.** One figure is held as a declared non-compliance and the other as a deliberate exception — **two is a pattern; a third arriving would make the closure the error rather than the exceptions.**

- **4 — The snapped 21, carried as the worked example rather than an open question.** It is already in the spec's table with its provenance. Keep it in front of the session because it is the case that argues for the table's existence: the figure was **ratified in an implementation render rather than a frozen one**, and expressed as a **`font-size` on a text span rather than an icon width** — so it sat in neither the ratified set nor the expected vocabulary, and **a figure in neither is invisible to a careful search**. And the relay that carried it into the session packet **named no file at all** — which is how a figure from outside the frozen set arrives looking like one from inside it.

- **5 — Open Flag to Rob (owner) for the next design session, facilitated by fable-design: A8 Explore's measured rows await wholesale ratification or amendment at that session; they are facts about the frozen render, not yet system law.**
