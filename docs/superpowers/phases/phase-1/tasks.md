...
T1.4 can guarantee 44×44pt targets for the controls it constructs, but an opaque caller-built SwiftUI or `UIViewRepresentable` control owns its own hit region. **Your custom injected controls must therefore prove 44×44pt at adoption.** In particular, replace `LocationSettingsButton`'s current fixed 68×16 frame with a genuine 44pt-minimum UIKit/SwiftUI target and retain `map.location-settings`; do the same for any T1.3-styled injected action. T1.4's package-test host also cannot surface the mounted SwiftUI button's accessibility element, so add app-level evidence that the whole-surface download control announces its derived progress value (for example `42%`) alongside its label and hint.
