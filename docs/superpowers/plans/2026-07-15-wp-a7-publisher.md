...
- **`current.schema.json`** (NEW) — the pointer contract B-track's tile client consumes: `{schema_version:1, publish_version}` (`publish_version` same pattern), `additionalProperties:false`. `current.json` is a new cross-track artifact (spec-fidelity MED) — it MUST be a pinned A0 contract, not an ad-hoc file. (Note: §5.6's "rollback = repoint the manifest" is realised as immutable-per-version manifests + a repointed `current` — the reinterpretation is stated.) **B3-consumer note: the tile client's manifest reader MUST gate on `min_reader_version` (refuse/upgrade when it exceeds the reader's version) — A0's `check_version` keys on `schema_version` ONLY, and `additionalProperties:false` is just the belt; the credits-rendering floor is enforced by B3 reading `min_reader_version`.**
- **`min_reader_version`**: a manifest carrying **non-empty** `attribution` sets `min_reader_version = 2` (spec-fidelity MED — attribution carries a LEGALLY REQUIRED credit; a reader that can't render credits must REFUSE/upgrade rather than ship HE/OSM data uncredited); only an attribution-LESS manifest (all sources CC0-without-courtesy, e.g. wikidata) stays `1` — a plaque-only region is reader 2 (Open Plaques PDDL carries a courtesy string).
...
- **BLOCKED-ON Rob's R2 token** — an S3-compatible token scoped to EXACTLY `mt-tiles` (public) + `mt-state` (private), read+write, in 1Password (`R2_S3_ENDPOINT`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`) + `.env.tpl`; bucket names stay in `pipeline/config/r2_layout.json`. One `op run` per publish run (`docs/SECRETS.md`).
