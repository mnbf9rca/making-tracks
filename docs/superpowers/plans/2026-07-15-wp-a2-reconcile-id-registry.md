...
- [ ] **Steps 2–4:** implement `reconcile.reconcile` per the interface. **Critical:** an existing-place match **reuses the stored `place_id` and `mint_anchor`** and only unions refs + advances `last_seen`; `mint_place_id` is called **only** on the `None` branch. Redirect-bridge builds a `bridged` view of existing refs (`canonicalize_ref` each) for the `resolve_by_refs` call, then writes the winner ref into the real record's union. `AmbiguousRefsError` → `review.append(...)`, no mutation. **Teeth (corrected — D3):** the "recompute the anchor each run" bug reds **`test_osm_gains_wikidata_tag_keeps_id` alone** (the new higher-priority ref flips the anchor → new id). It does **NOT** red `test_qid_merge_keeps_place_id_stable` (the union `{wd:Q123, wd:Q456}` still min-selects `wd:Q123`, re-deriving the same id) — that test's tooth is **breaking redirect-bridging** (identity `_bridge_refs` → the merged QID no longer matches → a spurious 2nd id is minted → its `len==1` assertion reds). Verify BOTH neuters against the real `mt_contracts` venv.
...
- `stages.run_stage(conn, region, "reconcile", *, run_id, version)` → calls `reconcile.run(conn, region, run_id=run_id, version=version)`:
  ...
  - **REDIRECT GATE — FAIL-CLOSED, CODE-ENFORCED (fixes the fail-open gap the gate found; the bypass-bar is enforcement, not prose — D1).** A run that stamps shippable ids **refuses** unless it has a redirect map that is **present, complete, and at least as fresh as the wikidata snapshot**: if `snap.snapshot_date` is blank (no real map) *or* `redirects.assert_fresh(snap, wikidata_snapshot_date)` raises → **abort**. The bypass is **not a flag on `mt reconcile`** — it is a **separate dev-only entrypoint** (`reconcile.run(..., allow_no_redirects=True)`, exercised only by dev tests), so the production CLI command literally cannot pass it. Reconcile **records the bridging state in run metadata** — `stage_runs`/the `ReconcileResult` carries `redirect_bridging ∈ {"active","inert"}` (`"active"` = a real fresh map bridged the refs; `"inert"` = the dev bypass) — and the **Task 9 report HARD-FAILS if `redirect_bridging != "active"`**. So the flagship run cannot silently reconcile with protection off: the gate refuses it, the CLI can't bypass it, and the report asserts it was active.
...
- **`SOURCE_KEY_TO_PREFIX` (fixes the dead namespace the gate found):** region-config source keys are `wikidata/wikipedia/osm/historic_england/open_plaques` (+ Malaysia's `national_register`); a place's member-sources are ref-prefixes `wd/wp/osm/hehle/plaque`. The stage maps the region's succeeded **keys** to **prefixes** — `{"wikidata":"wd","wikipedia":"wp","osm":"osm","historic_england":"hehle","open_plaques":"plaque"}` — and passes `succeeded_sources` **as ref-prefixes** to the core. **`national_register` is deliberately absent (spike-excluded, no ref prefix minted yet, disabled everywhere) — and the stage ASSERTS the map covers every *enabled* source key** (`set(enabled sources) - {"national_register"} ⊆ SOURCE_KEY_TO_PREFIX`), so a future enabled source with no mapping fails loudly rather than silently dead (D4 — the exact dead-namespace class the gate caught for `succeeded_sources`). A stage-level test builds `succeeded_sources` from real `RegionConfig.sources` names so the mapping is under test.
...
**Interfaces:** consumes Tasks 1–8. No new production code except the report script — this task **proves** the WP on real data.

**⛔ BLOCKED-ON `wp-acquire-impl` (D2 — declared, not discovered).** The real malaysia run cannot execute until codex's acquire deliverables land: (a) real malaysia `source_records` (a real extract) and (b) a **real, dated, complete redirect-map snapshot + the wikidata snapshot date** — both are codex's in-flight `wp-acquire-impl` scope. Because the redirect gate is **fail-closed** and `--allow-no-redirects` is barred from this path, Task 9 **must** run against really-acquired data; it does **not** run on fixtures and does **not** use the dev bypass. **Sequencing:** A2 impl proceeds through Tasks 1–8 (all fixture-testable) now; **Task 9 runs after `wp-acquire-impl` merges.** (The alternative — a hand-authored redirect-map fixture — is rejected: a fabricated-provenance map on the flagship run defeats the point of the freshness gate.)

- [ ] **Step 1: Determinism guard (executable NOW, on fixtures)** — a test that runs `reconcile.reconcile` twice on a shuffled copy of a multi-cluster fixture (QID joins + a fuzzy defer + an ambiguity) and asserts identical `records`/`places`/`review` (ids, order, refs). Reds if any ordering leaks (unsorted iteration, dict-order dependence).

- [ ] **Step 2: Real malaysia run (Rob's bar) — AFTER `wp-acquire-impl`.** With really-acquired data:
```bash
cd pipeline && uv run mt extract malaysia --run-id real1
uv run mt reconcile malaysia --run-id real1 --version 20260715T000000Z   # fail-closed gate must PASS (real fresh redirect map present)
uv run python scripts/reconcile_report.py malaysia
```
`scripts/reconcile_report.py` prints: **cluster count**; **place_ids minted** (count + sample); **QID-anchored vs singleton breakdown** (no fuzzy-merged bucket — fuzzy is defer-only); **review-file size** (ambiguities + fuzzy-defers); the **re-run-is-identical** confirmation (reconcile twice, diff the registry — must be empty); and — **D1 — it reads `redirect_bridging` from the run metadata and HARD-FAILS unless it is `"active"`** (proving the run bridged a real fresh redirect map, not ran with protection inert). Commit the output as `docs/superpowers/reports/2026-07-15-a2-malaysia-reconcile.md`.

- [ ] **Step 3: Commit** — `"Add reconcile determinism guard + real malaysia report (asserts redirect-bridging active; runs after wp-acquire-impl)"`
