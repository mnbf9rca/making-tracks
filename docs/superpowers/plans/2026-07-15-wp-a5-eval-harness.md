...
- **Human-editable + robust (robustness-to-human-edit — NOT the §5.5 untrusted-data posture).** The hand-label file is a **TSV** Rob edits by hand (name/category/score visible, one `label` column to fill). Rob's labels are *trusted-but-fallible*, not adversarial — so the tolerant loader is justified by fat-finger risk, not §5.5 (§5.5 governs *hostile external* source data — vandalised Wikipedia/OSM/C2 feedback). The loader is **tolerant**: identifies `place_id`/`label` **by header NAME, not end-relative position** (hostile-data critic HIGH-2); **pads/truncates each row to the header column count** (missing trailing cells → blank — so a freshly-dumped unlabeled row whose last cells are empty is NOT dropped; coherence critic MED); **normalises `label = raw.strip().lower()`** then validates `∈ {yes, meh, no, ""}` (blank → `None`; so `YES`/` Yes `/`no\t` survive — hostile-data MED-4); validates `place_id` via the **shipped `mt_contracts.place_id.is_valid_place_id` — NEVER a local regex** (hostile-data LOW-10; fable review D1: the shipped grammar is UPPERCASE Crockford excluding `I/L/O/U` and is `KNOWN_ID_SCHEMES`-aware, so a hand-rolled `^mt1_[0-9A-Za-z]{26}$` under-validates AND ignores scheme evolution — the consume-don't-reinvent rule this plan applies to `composite.score` applies to the oldest frozen contract too); **never crashes**.
...
from mt_contracts.place_id import is_valid_place_id           # the SHIPPED validator — never a local regex
from mt_pipeline.eval import golden as G

# Fixture ids MUST pass the real contract: UPPERCASE Crockford (0-9 A-Z excl I/L/O/U), 26-char body.
A = "mt1_" + "0" * 26                                         # is_valid_place_id(A) is True
B = "mt1_" + "1" * 26
C = "mt1_" + "2" * 26
Z = "mt1_" + "Z" * 26                                         # lexically LATER than A (used by the llm-flip fixture, Task 2)
assert all(map(is_valid_place_id, (A, B, C, Z)))             # tripwire: fixtures honour the frozen grammar

def test_loader_uses_the_shipped_place_id_validator():
    # Drift-guard (mirrors the composite.score consume-don't-reinvent test): a lowercase / I-L-O-U body
    # is REJECTED by the real contract, so a row bearing it must be a COUNTED skip — proving the loader
    # calls is_valid_place_id, not a permissive local regex that would accept it.
    assert not is_valid_place_id("mt1_" + "I" * 26) and not is_valid_place_id("mt1_" + "a" * 26)
    hdr = "\t".join(["place_id","name","lat","lon","category","tier","score","article","llm_curiosity","label"])
    bad = "# x\n# data_version: v1\n" + hdr + "\n" + ("mt1_" + "I" * 26) + "\tX\t0\t0\tc\t1\t0\t0\t\tyes\n"
    res = G.parse_labeled_tsv(bad)
    assert res.parsed == 0 and len(res.skipped) == 1 and "place_id" in res.skipped[0][1]
...
```
(`A`/`B`/`Z` are the Task-1 uppercase-Crockford fixture ids, valid under `is_valid_place_id`; `Z` sorts lexically after `A`.) **Teeth (host-verified):** `test_llm_off_flips_the_ranking` asserts the *ordering* changes, not dataclass inequality — it reds if `llm_on` is ignored OR if the writeback is dropped. The config test reds if weights aren't applied.
...
**fable independent review (thread `wp/a5`) — D1 (blocking) fixed:** I had hand-rolled the place_id grammar as `^mt1_[0-9A-Za-z]{26}$` (accepts lowercase + `I/L/O/U`, ignores `KNOWN_ID_SCHEMES` evolution) and my fixtures were lowercase `mt1_ + "a"*26` — **invalid under the shipped `mt_contracts.place_id.PLACE_ID_RE` (uppercase Crockford)**, so the moment an implementer correctly imports the real validator my own `parsed==2` tests would fail. The consume-don't-reinvent rule I wrote for `composite.score` applies to the *oldest* frozen contract too. **Fixed:** the loader validates via the shipped **`is_valid_place_id`** (never a local regex); fixtures are now valid uppercase-Crockford bodies (`mt1_ + "0"*26`…, host-verified against the real function); added a drift-guard test that a lowercase / `I`-body id is a counted skip — proving the import is the real validator. Everything else held under fable's verification (interior-k teeth, `min(k,n)` divisor, `union(active,retired)` resurrection, data_version-in-TSV, composite-signature match against A4's merged plan, the A6 seam, golden-areas config, honest sequencing).
