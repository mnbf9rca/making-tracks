...
**Cross-cutting:** the DATA-not-instructions framing (point 1a/2a) is added to `render_prompt` NOW (Task 3, applies to every task), and the injection-resistance dimension + fixture (point 1c) is built NOW into the bake-off (Task 5, keyless). Blurb content-safety (point 2) and adjudication (point 3) are security *requirements* pinned onto their later WPs, not built here.
