...
fills `accent`, **loved** fills the ratified `love` row, and **saved** fills an **accent-family
dark fill carrying `accentContrast` ink** *(amended by designer ruling 2026-07-30;
"tonal-strength companion" is retired from this rider — same-ink ON/OFF pairs cap at 1.43:1
separation by arithmetic for a free pair, lower still once either value is pinned, so saved adopts the two-ink inversion seen and loved already use)*. **Exact values are
...
|---|---|---|
| Door pill | 44pt height, 999px radius, 15pt/600 label, 7pt gap, **15×15 (`IconRole.inline`) icon**, 10pt between doors | Designer ruling #594, relayed by Rob 2026-08-01 — "R11 snap from ia-doors' drawn 19, tolerance proven by review acceptance," |
