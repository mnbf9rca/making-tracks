...
- A hostile same-UID process could still replace the quarantine pathname after its final revalidation and before recursive unlink; `docs/threat-model.md` excludes our own infrastructure turning hostile, and that actor could already edit the scripts or delete arbitrary files directly, so #546 does not add a compiled fd-relative deletion helper for this non-boundary.
