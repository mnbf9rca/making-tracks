...
Issue #600's cap-2 measurements exposed two independent harness races and review of those paths found two sibling blind-wait seams.
...
The generic existence helper retained the same opaque five-swipe loop at eight live call sites. Separately, the focused morphology test failed twice after all nine default exports because its shared fixture opener tapped the map centre before the fixture source and accessibility pin were ready. The preserved pre-fix result bundle is `/private/tmp/release-gate-607-morphology-rerun.xcresult`.

## Chosen Design

### Artifact directory selection

Introduce a pure selector that accepts an optional simulator identity and returns the artifact directory. Every artifact uses `/private/tmp/making-tracks-artifacts.<simulator-id>` whenever a non-empty, whitespace-trimmed identity is available. Runtime identity keeps the existing precedence: `SIMULATOR_UDID`, then `MT_SIM_LOCK_UDID`. If neither is present, exports deliberately retain the documented single-process fallback `/private/tmp/making-tracks-artifacts`.
...
The generic existence helper delegates to the same bounded acquirer, mapping existence to its terminal match state while preserving its established five-scroll budget. This removes the sibling blind loop without changing its callers' contract.

### Condition-driven fixture opening

The shared primary-fixture opener first waits for the fixture reset's expected hidden state, then observes the stable `map.pin.<place-id>` accessibility button. That element is emitted only after the fixture reaches the map's accessibility source, so it is the condition the opener actually needs and works in production-like render launches where debug-readiness markers are deliberately absent. At accessibility text sizes the enlarged test fixture controls cover the map centre; when the named pin is present but non-hittable, the opener performs at most three map repositions through an uncovered edge, observes the pin again, and taps only after it becomes hittable. Failure output records the expected reset state, pin identifier, bounded count, final observation, and frame. The opener does not enable the visible pin-diagnostics overlay, which would contaminate morphology screenshots.
...
- nil, empty, and whitespace-only identities select the documented fallback;
- `SIMULATOR_UDID` wins when both supported identity variables are present;
...
The focused integration gate runs the custom-list test and the place-card morphology export test. The morphology test's repeated pre-fix opener failure is the RED evidence for condition-driven fixture opening. Mutation evidence must show the selector tests fail if simulator scoping is removed and the scroll tests fail if the bound is not enforced or a non-hittable observation is accepted.
