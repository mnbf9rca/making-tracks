...
| Wikipedia page records | `pipeline/config/acquire_sources.json`; `pipeline/src/mt_pipeline/extractors/wikipedia.py`; source refs `wp:*` | Most Wikimedia text: [CC BY-SA 4.0 International](https://creativecommons.org/licenses/by-sa/4.0/) and GFDL where applicable; media and page content can vary by item | Current A7 publish emits no Wikipedia article text, excerpts, media, `wikipedia_title`, `blurb`, or `image_url`; only canonical `wp:*` refs can appear via `source_refs`. Shipping Wikipedia-derived text/media requires a separate `wikipedia` attribution source, `wp` mapping, attribution text, and tests before release. |
